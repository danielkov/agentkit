...
| -------------------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `agent.turn`         | `agentkit_loop` | `otel.name="invoke_agent"`, `gen_ai.operation.name="invoke_agent"`, `gen_ai.conversation.id`, `gen_ai.provider.name`, `session.id`, `turn.id`, `transcript.len`, `saw_tool_call`, `finish_reason`    |
...
With the `agentkit-loop/otel` feature, token usage is exported as signed 64-bit attributes and provider-native finish reasons are exported as a stable, deduplicated string array. Usage, finish-reason, and message attributes are then written directly to the OpenTelemetry span and are not visible to a plain `tracing-subscriber` formatting layer. Token counts outside the OpenTelemetry `i64` range are omitted with a warning. Usage is chat-only: final usage fields win, while the latest streamed usage fills only missing tokens, cost, and metadata. The `gen_ai.usage.cost` F64 attribute is an extension documented by the [Open LLM Observability RFC](https://github.com/sauravGit/open-llm-observability/blob/main/RFC.md). Existing adapters can retain native reasons without changing `ModelTurnResult` by calling `set_provider_finish_reasons(&mut result.metadata, reasons)`.

Message capture is off by default and never reads `OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT`. Configure input and output independently with validated `MessageCapture::new(max_messages, max_bytes)` values passed to `TelemetryConfig::with_input_messages` and `with_output_messages`. Zero limits are errors and values are never silently clamped. Each captured attribute is an OpenTelemetry `Array<String>` whose elements are compact valid JSON. Input capture keeps the newest bounded tail in order; output capture keeps the bounded head. Binary, image, and audio data and all data-reference URLs are omitted and never dereferenced. Oversized items use a structured JSON truncation record, even for tiny byte budgets.

`ModelSession::provider_name()` can report the concrete provider for an active session and takes precedence over the backward-compatible adapter-level identity; its default is `None`.

`launch_kind` is `"plain"` for tool calls dispatched in a normal tool round, `"approved"` when the call resumes after a human-in-the-loop approval. `gen_ai.provider.name` is sourced from `ModelSession::provider_name()` with `ModelAdapter::provider_name()` as a fallback, and `gen_ai.request.model` from `ModelSession::model_name()`.
